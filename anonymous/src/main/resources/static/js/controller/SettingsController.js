class SettingsController {
	...
	constructor() {
		this.columns = [
			...
			{ name: 'groupValue', label: 'Group Value', type: 'text', required: true },
			{ name: 'orderId', label: 'Order id', type: 'numeric', required: false }
			// Add more columns as needed
		];


		this.apiUrl = '/restCtrSettings'; // Replace with your API endpoint
	...
	fetchData() {
		...
		const groupValue = document.getElementById('groupValue').value.trim();

		const url = `${this.apiUrl}?process=${processFilter}&groupValue=${groupValue}`;
	...
	getItemById(itemId) {
		...
		const item = {
			...
			group: document.getElementById('group' + itemId).value,
			key: document.getElementById('key' + itemId).value,	
			value: document.getElementById('value' + itemId).value,
			groupValue: document.getElementById('groupValue' + itemId).value,
			orderId: document.getElementById('orderId' + itemId).value,
